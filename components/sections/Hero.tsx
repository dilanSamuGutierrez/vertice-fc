...
export default function Hero() {
  ...
  return (
    <section ref={ref} onMouseMove={onMove} className="relative min-h-[100svh] overflow-hidden bg-night">
      ...
      {/* Floating depth orbs */}
      <motion.div style={{ x: tx1, y: ty1 }} className="absolute right-[8%] top-[22%] h-40 w-40 rounded-full bg-lime/20 blur-3xl md:h-72 md:w-72" />
      <motion.div style={{ x: tx2, y: ty2 }} className="absolute left-[5%] bottom-[18%] h-48 w-48 rounded-full bg-grass/30 blur-3xl md:h-80 md:w-80" />

      {/* Content */}
      <motion.div style={{ y: yText, opacity }} className="relative z-10 mx-auto flex min-h-[100svh] max-w-container flex-col justify-end px-5 pb-16 pt-32 md:px-8 md:pb-24">
        <motion.span
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 1.7, duration: 0.7 }}
          className="mb-6 inline-flex w-fit items-center gap-2 rounded-full border border-lime/30 bg-lime/10 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-lime"
        >
          <span className="h-2 w-2 animate-pulse rounded-full bg-lime" /> Academia de alto rendimiento · Medellín
        </motion.span>

        <h1 className="font-display uppercase leading-[0.85] tracking-huge text-chalk">
          {["DONDE EL TALENTO", "SE CONVIERTE", "EN OFICIO"].map((line, i) => (
            <span key={i} className="block overflow-hidden">
              <motion.span
                initial={{ y: "110%" }}
                animate={{ y: 0 }}
                transition={{ delay: 1.75 + i * 0.12, duration: 0.85, ease: [0.16, 1, 0.3, 1] }}
                className={`block text-[15vw] md:text-[8.5vw] xl:text-[9rem] ${i === 1 ? "text-grad-lime" : ""}`}
              >
                {line}
              </motion.span>
            </span>
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-4">
            <MagneticButton href="#registro" variant="lime">Agenda tu prueba gratis</MagneticButton>
            <MagneticButton href="#programas" variant="outline">Ver programas</MagneticButton>
          </div>
        </motion.div>
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute bottom-6 left-1/2 z-10 hidden -translate-x-1/2 flex-col items-center gap-2 text-xs uppercase tracking-widest text-ash md:flex"
      >
        Scroll
